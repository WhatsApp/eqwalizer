...
public class OtpOutputStream extends ByteArrayOutputStream {
  ...
  /**
   * Trims the capacity of this <code>OtpOutputStream</code> instance to be the buffer's current
   * size. An application can use this operation to minimize the storage of an <code>OtpOutputStream
   * </code> instance.
   */
  ...
  /**
   * Increases the capacity of this <code>OtpOutputStream</code> instance, if necessary, to ensure
  ...
  /**
   * Get the current capacity of the stream. As bytes are added the capacity of the stream is
   * increased automatically, however this method returns the current size.
   *
  ...
  /**
   * Write the low four bytes of a value to the stream in bif endian order, at the specified
   * position. If the position specified is beyond the end of the stream, this method will have no
   * effect.
   *
   * <p>Normally this method should be used in conjunction with {@link #size() size()}, when is is
   * necessary to insert data into the stream before it is known what the actual value should be.
   * For example:
   *
   * <pre>
   * int pos = s.size();
   ...
   *
   * @param bin the array of bytes to write.
   * @param pad_bits the number of zero pad bits at the low end of the last byte
   */
  ...
  /**
   * Write a single byte to the stream as an Erlang integer. The byte is really an IDL 'octet', that
   * is, unsigned.
   *
  ...
  /**
   * Write a character to the stream as an Erlang integer. The character may be a 16 bit character,
   * kind of IDL 'wchar'. It is up to the Erlang side to take care of souch, if they should be used.
   *
  ...
  /**
   * Write a positive long to the stream. The long is interpreted as a two's complement unsigned
   * long even if it is negative.
   *
  ...
  /**
   * Write a positive integer to the stream. The integer is interpreted as a two's complement
   * unsigned integer even if it is negative.
   *
  ...
  /**
   * Write a positive short to the stream. The short is interpreted as a two's complement unsigned
   * short even if it is negative.
   *
  ...
  /**
   * Write an Erlang list header to the stream. After calling this method, you must write 'arity'
   * elements to the stream followed by nil, or it will not be possible to decode it later.
   *
  ...
  /**
   * Write an Erlang tuple header to the stream. After calling this method, you must write 'arity'
   * elements to the stream or it will not be possible to decode it later.
   *
   ...
   *
   * @param o the Erlang term to write.
   * @param level the compression level (<code>0..9</code>)
   */
  public void write_compressed(final OtpErlangObject o, final int level) {
    ...
    if (oos.size() < 5) {
      ...
      final java.util.zip.DeflaterOutputStream dos =
          new java.util.zip.DeflaterOutputStream(this, def);
      try {
