...
/**
 * Provides a Java representation of Erlang refs. There are two styles of Erlang refs, old style
 * (one id value) and new style (array of id values). This class manages both types.
 */
public class OtpErlangRef extends OtpErlangObject {
   ...
   * @exception OtpErlangDecodeException if the buffer does not contain a valid external
   ...
   *
   * @param node the nodename.
   * @param ids an array of arbitrary numbers. Only the low order 18 bits of the first number will
   *     be used. If the array contains only one number, an old style ref will be written instead.
   *     At most three numbers will be read from the array.
   * @param creation node incarnation number. Avoid values 0 to 3.
   */
   ...
   *
   * @param tag the external format to be compliant with. OtpExternal.newRefTag where only a subset
   *     of the bits are used (see other constructor) OtpExternal.newerRefTag where all bits of ids
   *     and creation are used. newerPortTag can only be decoded by OTP-19 and newer.
   * @param node the nodename.
   * @param ids an array of arbitrary numbers. At most three numbers will be read from the array.
   * @param creation another arbitrary number.
   */
  public OtpErlangRef(final int tag, final String node, final int[] ids, final int creation) {
    this.node = node;
  ...
  /**
   * Get the id number from the ref. Old style refs have only one id number. If this is a new style
   * ref, the first id number is returned.
   *
  ...
  /**
   * Get the array of id numbers from the ref. If this is an old style ref, the array is of length
   * 1. If this is a new style ref, the array has length 3-5.
   *
   ...
   *
   * @param buf an output stream to which the encoded ref should be written.
   */
  @Override
  ...
  /**
   * Determine if two refs are equal. Refs are equal if their components are equal. New refs and old
   * refs are considered equal if the node, creation and first id number are equal.
   *
