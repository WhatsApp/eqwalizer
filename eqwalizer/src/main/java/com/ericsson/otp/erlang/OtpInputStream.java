 ...
 *
 * <p>Note that this class is not synchronized, if you need synchronization you must provide it
 * yourself.
 */
public class OtpInputStream extends ByteArrayInputStream {
   ...
   * @param flags
   */
  public OtpInputStream(final byte[] buf, final int offset, final int length, final int flags) {
   ...
   *
   * @param pos the position to move to in the stream. If pos indicates a position beyond the end of
   *     the stream, the position is move to the end of the stream instead. If pos is negative, the
  ...
  /**
   * Read an array of bytes from the stream. The method reads at most buf.length bytes from the
   * input stream.
   *
   * @return the number of bytes read.
   * @exception OtpErlangDecodeException if the next byte cannot be read.
   */
  ...
  /**
   * Read an array of bytes from the stream. The method reads at most len bytes from the input
   * stream into offset off of the buffer.
   *
   * @return the number of bytes read.
   * @exception OtpErlangDecodeException if the next byte cannot be read.
   */
   ...
   *
   * @return true if the atom at the current position in the stream contains the value 'true'
   *     (ignoring case), false otherwise.
   * @exception OtpErlangDecodeException if the next term in the stream is not an atom.
   */
   ...
   * @exception OtpErlangDecodeException if the next term in the stream is not a binary.
   */
   ...
   *
   * @param pad_bits an int array whose first element will be set to the number of pad bits in the
   *     last byte.
   * @return a byte array containing the value of the bitstr.
   * @exception OtpErlangDecodeException if the next term in the stream is not a bitstr.
   */
  public byte[] read_bitstr(final int pad_bits[]) throws OtpErlangDecodeException {
    ...
    if (tag != OtpExternal.bitBinTag) {
      throw new OtpErlangDecodeException(
          "Wrong tag encountered, expected " + OtpExternal.bitBinTag + ", got " + tag);
    }
   ...
   * @exception OtpErlangDecodeException if the next term in the stream is not a float.
   */
  ...
  public double read_double() throws OtpErlangDecodeException {
    ...
    switch (tag) {
      ...
      default:
        throw new OtpErlangDecodeException(
            "Wrong tag encountered, expected " + OtpExternal.newFloatTag + ", got " + tag);
    }
  }
   ...
   * @exception OtpErlangDecodeException if the next term in the stream is not an integer that can
   ...
   * @exception OtpErlangDecodeException if the next term in the stream cannot be represented as a
   *     positive integer.
   */
   ...
   *
   * @return the integer value.
   * @exception OtpErlangDecodeException if the next term in the stream cannot be represented as an
   *     integer.
   */
   ...
   * @exception OtpErlangDecodeException if the next term in the stream is not an integer.
   */
    ...
    switch (tag) {
      ...
      case OtpExternal.largeBigTag:
        ...
        if (tag == OtpExternal.smallBigTag) {
          ...
          if (arity + 1 < 0) {
            throw new OtpErlangDecodeException(
                "Value of largeBig does not fit in BigInteger, arity " + arity + " sign " + sign);
          }
        }
        nb = new byte[arity + 1];
   ...
   * @exception OtpErlangDecodeException if the next term in the stream is not a list.
   */
   ...
   * @exception OtpErlangDecodeException if the next term in the stream is not a tuple.
   */
   ...
   * @exception OtpErlangDecodeException if the next term in the stream is not an empty list.
   */
   ...
   * @exception OtpErlangDecodeException if the next term in the stream is not a string.
   */
   ...
   * @exception OtpErlangDecodeException if the next term in the stream is not a compressed term.
   */
   ...
   * @exception OtpErlangDecodeException if the stream does not contain a known Erlang type at the
   *     next position.
   */
