...
/**
 * Base class of the Erlang data type classes. This class is used to represent an arbitrary Erlang
 * term.
 */
public abstract class OtpErlangObject implements Serializable, Cloneable {
   ...
   *
   * @param buf an input stream containing one or more encoded Erlang terms.
   * @return an object representing one of the Erlang data types.
   * @exception OtpErlangDecodeException if the stream does not contain a valid representation of an
   *     Erlang term.
   */
  public static OtpErlangObject decode(final OtpInputStream buf) throws OtpErlangDecodeException {
    return buf.read_any();
  }

  /**
   * Determine if two Erlang objects are equal. In general, Erlang objects are equal if the
   * components they consist of are equal.
   *
   ...
   *
   * @param term the object to match
   * @param binds variable bindings
