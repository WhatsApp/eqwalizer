...
/**
 * Provides a Java representation of Erlang lists. Lists are created from zero or more arbitrary
 * Erlang terms.
 *
...
public class OtpErlangList extends OtpErlangObject implements Iterable<OtpErlangObject> {
  ...
  /**
   * Create a list of Erlang integers representing Unicode codePoints. This method does not check if
   * the string contains valid code points.
   *
  ...
  /**
   * Create a list from an array of arbitrary Erlang terms. Tail can be specified, if not null, the
   * list will not be proper.
   *
   * @param elems array of terms from which to create the list
   * @param lastTail
   ...
   *
   * @param i the index of the requested element. List elements are numbered as array elements,
   *     starting at 0.
  ...
  /**
   * Convert a list of integers into a Unicode string, interpreting each integer as a Unicode code
   * point value.
   *
   * @return A java.lang.String object created through its constructor String(int[], int, int).
   ...
   * @exception java.security.InvalidParameterException if any integer is not within the Unicode
   *     range.
