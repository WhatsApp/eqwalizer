...
/**
 * Provides a Java representation of Erlang PIDs. PIDs represent Erlang processes and consist of a
 * nodename and a number of integers.
 */
public class OtpErlangPid extends OtpErlangObject implements Comparable<Object> {
   ...
   * @param serial another arbitrary number. Only the low order 13 bits will be used.
   * @param creation node incarnation number. Avoid values 0 to 3.
   */
  public OtpErlangPid(final String node, final int id, final int serial, final int creation) {
   ...
   * Create an Erlang pid from its components.
   *
   * @param tag the external format to be compliant with OtpExternal.pidTag where only a subset of
   *     the bits are significant (see other constructor). OtpExternal.newPidTag where all 32 bits
   *     of id,serial and creation are significant. newPidTag can only be decoded by OTP-19 and
   *     newer.
   * @param node the nodename.
   * @param id an arbitrary number.
   * @param serial another arbitrary number.
   * @param creation yet another arbitrary number.
   */
  protected OtpErlangPid(
      final int tag, final String node, final int id, final int serial, final int creation) {
    this.node = node;
   ...
   *
   * @param buf an output stream to which the encoded PID should be written.
   */
  @Override
   ...
   *
   * @param o the other PID to compare to.
