...
public class OtpErlangPort extends OtpErlangObject {
   ...
   * Create an Erlang port from its components.
   *
   * @param tag the external format to be compliant with. OtpExternal.portTag where only a subset of
   *     the bits are used (see other constructor) OtpExternal.newPortTag where all 32 bits of id
   *     and creation are significant. newPortTag can only be decoded by OTP-19 and newer.
   * @param node the nodename.
   * @param id an arbitrary number. Only the low order 28 bits will be used.
   * @param creation another arbitrary number.
   */
  public OtpErlangPort(final int tag, final String node, final long id, final int creation) {
    this.node = node;
   ...
   *
   * @param buf an output stream to which the encoded port should be written.
   */
  @Override
  ...
  public boolean equals(final Object o) {
    ...
    return creation == port.creation && id == port.id && node.compareTo(port.node) == 0;
  }
